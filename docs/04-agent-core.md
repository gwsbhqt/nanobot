...
```python
class AgentLoop:
    def __init__(self, bus, provider, workspace, model, simple_model, complex_model, model_routing="auto", max_iterations=40,
        ...
        self.simple_model = simple_model  # 简单模型（低成本）
        self.complex_model = complex_model  # 复杂模型（高能力）
        self.model_routing = model_routing  # auto/simple/complex
...
async def _run_agent_loop(self, initial_messages, on_progress):
    ...
    iteration = 0
    active_model = self._pick_initial_model(user_message)  # 默认简单模型，按规则升级
    ...
    while iteration < self.max_iterations:
        ...
        if response.has_tool_calls:
            ...
            for tool_call in response.tool_calls:
                ...
                # 添加 tool result 消息
            # auto 模式下，工具错误连续出现或迭代过多会升级到复杂模型
            # 继续循环
        else:
            # 无工具调用 = 最终响应
...
```jsonl
{"key": "feishu:ou_xxx", "created_at": "2024-01-15T10:00:00", "last_consolidated": 42}
